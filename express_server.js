...
const postOrPutUser = (method, req, res) => {
  ...
  if (id && email && password) {
    ...
    for (const key of keys) {
      if (
        (method === "post" && users[key].email === email) ||
  ...
  if (alert) {
    res.redirect("/register"); //An error occured Restart the register
  } else {
    res.redirect("/urls");
  }
};
...
const postOrPutURL = (method, req, res) => {
  ...
  if (longurl) {
    //URL not empty or else do nothing
    ...
    for (const url in urlDatabase) {
      if (
        (method === "put" &&
          urlDatabase[url].longURL === longurl &&
          url !== shorturl) ||
...
//Router used to display either register.ejs to edit a profile or
//                              register_new.ejs to register a new profile
app.get("/register", (req, res) => {
  ...
  if (email) {
    res.render("register", templateVars); //Edit existing profile
  } else {
    res.render("register_new", templateVars); //Create new profile
  }
});
